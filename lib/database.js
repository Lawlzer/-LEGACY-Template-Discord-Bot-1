...
exports.returnNerdDumpTimeOut = function () {
  return nerdDumpTimeOut; // simply return the nerd dump time out in seconds
  // this is needed in index.js, if the user reacts to a message that we no longer save the information for
}
...
exports.sendEmbed = async function (bot, message, title, description) {
  ...
  var initialMessageSentTime = Date.now();
  var messageRecievedByBotTime = message.messageRecievedByBotTime;
  message.channel.send(basicEmbed).then(msg => {
    ...
    const botSecondTime = Date.now();
    var currentEmbedTemporaryInformation = {
      id: msg.id,
      discordInitialMessageTime: message.createdTimestamp,
      discordSecondMessageTime: msg.createdTimestamp,
      initialMessageStarted: messageRecievedByBotTime,
      initialMessageSent: initialMessageSentTime,
