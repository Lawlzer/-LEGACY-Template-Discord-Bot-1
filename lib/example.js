...
exports.setCommand = async function (bot, message, argsLowercase, content) {
    ...
    var argsOriginalCase = content.split(' '); // we want to save the command with caps, so we need to make normal args
    var commandTriggerOriginalCase = argsOriginalCase[0]; // the first word is the trigger word
    argsOriginalCase.slice(1); 
    
    argsOriginalCase.shift();
    var commandDescriptionOriginalCase = argsOriginalCase;
    ...
    for (let i = 0; i < currentCommands.length; i++) {
        ...
        if (commandTrigger == currentCommands[i].commandTrigger.toLowerCase()) {
    ...
    const newCommand = {
        commandTrigger: commandTriggerOriginalCase,
        commandDescription: commandDescriptionOriginalCase.join(' '),
    }
...
exports.findUserCommand = async function (bot, message, database, inputCommand) {
    var allCommands = database.userCreatedCommands;
    for (let i = 0; i < allCommands.length; i++) {
        console.log(allCommands[i]); 
        // inputCommand is already lowerCase
        console.log(allCommands[i].commandTrigger.toLowerCase()); 
        if (inputCommand == allCommands[i].commandTrigger.toLowerCase()) {
...
exports.createPoll = async function (bot, message, argsLowercase, content) {
    const messageContent = content; // Please use this variable, if this variable is modified and not updated lower down, the bot won't be able to react
    // because reacting is based on the content ID in newPoll
    await databaseController.sendEmbed(bot, message, 'Poll', messageContent);
    var database = await databaseController.getOrCreateDatabase(message.guild.id); 
    var allPolls = database.polls;
    if (!allPolls) { allPolls = []; console.log('all polls thing done'); }
